...
const SearchBar = ({ onsearch, onlocationSearch, searchCities, loading }) => {
    ...
    <div className='relative w-full max-w-2xl' ref={searchRef}>
      <form className='relative' onSubmit={handleSubmit}>
        <div className='relative group'>
          ...
          <input type="text" value={query} onChange={(e) => setQuery(e.target.value)} placeholder='Search city...' className='w-full pl-12 pr-24 py-4 bg-white/10 backdrop-blur-xl border border-white/20 rounded-2xl text-white placeholder-white/50 focus:outline-none focus:ring-2 focus:ring-white/30 focus:border-white/40 transition-all duration-300 hover:bg-white/15' />
          {/* Conditional Rendering */}
          {query && (
            <button type='button' onClick={clearSearch} className='absolute right-14 top-1/2 transform -translate-y-1/2 text-white/50 hover:text-white transition-all p-1 rounded-full hover:bg-white/10'>
              <X className="w-4 h-4" />
            </button>
          )}
